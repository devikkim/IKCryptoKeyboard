...
extension IKCryptoKeyboardView: IKCryptoButtonDelegate {
  func touchedKey(key: String) {
    self.plain += key
    self.delegate?.touched(keys: self.plain)
  }
...
extension IKCryptoKeyboardView {
  ...
  @IBAction func spaceTouch(sender:UIButton){
    self.plain += " "
    self.delegate?.touched(keys: plain)
  }
  
  @IBAction func backspaceTouch(sender:UIButton){
    if self.plain.count != 0 {
      let endIndex = plain.index(plain.endIndex, offsetBy: -1)
      self.plain = String(plain[..<endIndex])
      self.delegate?.touched(keys: plain)
    }
  }
